...
# convert pop from monthly to quarterly observations
...
class SimpleRBC(sm.tsa.statespace.MLEModel):
    # child class(parent class): inherit methods from parent's class

    parameters = OrderedDict([('discount_rate', 0.95),
                              ('disutility_labor', 3.),
    ...
    def __init__(self, endog, calibrated=None):
        ...
        super(SimpleRBC, self).__init__(endog, k_states=2,
                                        k_posdef=1, initialization='stationary')
        ...
        self.calibrated = OrderedDict(
            [(param, calibrated[param]) for param in parameters if param in calibrated])
        ...
        # setup the fixed elements of system matrices
        self['selection', 1, 0] = 1

    @property
    def start_params(self):
        structural_params = np.array(list(self.parameters.values()))[
            self.idx_estimated]
    ...
    def param_names(self):
        structural_params = np.array(list(self.parameters.keys()))[
            self.idx_estimated]
...
calibrated_irfs = calibrated_res.impulse_responses(
    40, orthogonalized=True) * 100
...
def plot_irfs(irfs):
    fig, ax = plt.subplots(figsize=(13, 2), dpi=100)
...
# http://www.chadfulton.com/topics/simple_rbc.html
# ====================================================================================================
